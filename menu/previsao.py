...
# def render():
#     st.header("Estimativa da MAP a partir de sensores R")
    
#     dfs_r, df_vitals, vitals_filtered = carregar_dados()
#     sensores = [k for k in dfs_r.keys() if not k.endswith('_filtered')]
    
#     # # Selector de sensor R
#     # sensor_sel = st.selectbox("Selecione o sensor R para estimativa da MAP", sensores, index=min(2, len(sensores)-1))


#     # Crear dos columnas para el selector y el desempeño
#     col_selector, col_metrics = st.columns([1, 2])
...
#     with col_metrics:
#         # Expander para métricas y desempeño
#         with st.expander("Métricas de Desempenho do Modelo"):
...
#             # Coeficientes de Determinação das Variáveis (nombre más técnico)
#             st.write("#### Coeficientes de Determinação das Variáveis")
...
#     if len(df_merge) < 10:
#         st.error("""
#         ❌ Erro: Não há dados suficientes para criar um modelo após a filtragem e alinhamento temporal.
...
#     # Divisão treino/validação/teste (ajustável com base no tamanho dos dados)
#     try:
...
#         with col1:
#             # st.subheader("Comparação: MAP real vs estimada")
#             st.markdown("<h3 style='text-align: center;'>Comparação: MAP real vs estimada</h3>", unsafe_allow_html=True)
...
#         with col2:
#             # st.subheader("MAP real vs estimada no tempo")
#             st.markdown("<h3 style='text-align: center;'>MAP real vs estimada no tempo</h3>", unsafe_allow_html=True)
...
# def render():
#     st.markdown("<h2 style='text-align: center;'>Estimativa da MAP a partir de sensores R</h2>", unsafe_allow_html=True)
...
#         sensor_sel = st.selectbox("Selecione o sensor R para estimativa da MAP", sensores, index=default_index)
...
#         # Ahora que tenemos todas las variables definidas, podemos mostrar el expander
#         with col_metrics:
#             # Expander para métricas y desempeño
#             with st.expander("Métricas de Desempenho do Modelo"):
#                 # Métricas
...
#                 st.markdown("<h4 style='font-size: 20px;'>Métricas de Avaliação</h4>", unsafe_allow_html=True)
...
#                 # Coeficientes de Determinação das Variáveis
#                 st.markdown("<h4 style='font-size: 20px;'>Coeficientes de Determinação das Variáveis</h4>", unsafe_allow_html=True)
...
#                     }),
#                     hide_index=True,
#                     height=100  # Ajustar altura para mejor visualización
#                 )
...
#     # Selector de sensor R en una sola línea
#     default_index = min(2, len(sensores)-1)
#     sensor_sel = st.selectbox("Selecione o sensor R para estimativa da MAP", sensores, index=default_index)
...
#         # Métricas de desempeño después de los gráficos
#         st.markdown("<h3 style='text-align: center;'>Métricas de Desempenho do Modelo</h3>", unsafe_allow_html=True)
...
def render():
    ...
    try:
        ...
        with col_test_metrics:
            ...
            # st.markdown(
            #     f"<div style='font-size: 16px; padding-top: 25px;'><b>Métricas de teste:</b> MSE: {mse_test:.2f} | RMSE: {rmse_test:.2f} | R²: {r2_test:.2f}</div>", 
            #     unsafe_allow_html=True
            # )
            st.markdown(
                f"<div style='font-size: 16px; padding-top: 25px; text-align: center;'><b>Métricas de teste:</b> MSE: {mse_test:.2f} | RMSE: {rmse_test:.2f} | R²: {r2_test:.2f}</div>", 
                unsafe_allow_html=True
            )
        ...
        st.markdown("<h3 style='text-align: center;'>Métricas do modelo</h3>", unsafe_allow_html=True)
